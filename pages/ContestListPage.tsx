          ...
          <div className="max-w-5xl mx-auto animate-fade-in bg-white rounded-[3rem] shadow-2xl border border-gray-100 overflow-hidden">
              ...
              <div className="p-10 lg:p-14 grid lg:grid-cols-2 gap-16">
                  <div className="space-y-12">
                          ...
                          <div className="grid grid-cols-3 gap-4">
                              {[Mode.VISUAL, Mode.LISTENING, Mode.FLASH].map(m => (
                                  <button key={m} onClick={() => setPracticeMode(m)} className={`p-6 rounded-[2rem] border-2 flex flex-col items-center gap-2 transition-all ${practiceMode === m ? 'border-ucmas-blue bg-blue-50 text-ucmas-blue shadow-lg scale-105' : 'border-gray-50 text-gray-300 hover:bg-gray-50'}`}>
                                      ...
                                      <span className="text-[10px] font-heading font-black uppercase tracking-tighter">{m === Mode.VISUAL ? 'Nhìn' : m === Mode.LISTENING ? 'Nghe' : 'Flash'}</span>
                                  </button>
                              ))}
                          </div>
                      </div>
                      ...
                      <div className="space-y-8">
                          <div className="space-y-4">
                              <label className="text-[11px] font-heading font-black text-gray-400 uppercase tracking-widest ml-1 block">CHỌN SỐ CHỮ SỐ (DIGITS)</label>
                              <div className="flex flex-wrap gap-3">
                                  {[1, 2, 3, 4, 5, 6, 7, 8].map(d => (
                                      <button 
                                        key={d} 
                                        onClick={() => setPracticeConfig({...practiceConfig, digits: d})} 
                                        className={`w-12 h-12 rounded-2xl font-heading font-black text-lg transition-all shadow-sm ${practiceConfig.digits === d ? 'bg-ucmas-blue text-white shadow-ucmas-blue/30 shadow-lg scale-110' : 'bg-gray-50 text-gray-400 hover:bg-gray-100'}`}
                                      >
                                        {d}
                                      </button>
                                  ))}
                          ...
                          <div className="space-y-4">
                              <label className="text-[11px] font-heading font-black text-gray-400 uppercase tracking-widest ml-1 block">CHỌN SỐ DÒNG (ROWS)</label>
                              <div className="grid grid-cols-6 gap-3">
                                  {[2, 3, 4, 5, 6, 7, 8, 9, 10, 12, 15, 20].map(r => (
                                      <button 
                                        key={r} 
                                        onClick={() => setPracticeConfig({...practiceConfig, operands: r})} 
                                        className={`h-12 rounded-2xl font-heading font-bold text-sm transition-all shadow-sm ${practiceConfig.operands === r ? 'bg-ucmas-blue text-white shadow-ucmas-blue/30 shadow-lg' : 'bg-gray-50 text-gray-400 hover:bg-gray-100'}`}
                                      >
                                        {r}
                                      </button>
                                  ))}
                  ...
                  <div className="space-y-12">
                      ...
                      <button onClick={startCustomPractice} className="group w-full relative h-24 bg-gradient-to-br from-ucmas-red to-ucmas-red/90 text-white rounded-[2rem] font-heading-extrabold text-2xl shadow-xl hover:shadow-2xl transition-all overflow-hidden active:scale-95">
              ...
              {assignedExams.length === 0 ? (
                <div className="col-span-full py-20 text-center bg-gray-50 rounded-[3rem] border-2 border-dashed border-gray-200">
                  ...
                  <div key={ex.id} className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm group hover:border-ucmas-blue transition-all relative overflow-hidden flex flex-col min-h-[220px]">
                      ...
                      <h4 className="text-xl font-heading font-black text-gray-800 mb-6 line-clamp-2">{ex.name}</h4>
                      <div className="flex gap-2 mb-8 mt-auto">
                         {ex.mode === Mode.MIXED || ex.config?.isMixed ? (
                             <span className="text-[10px] font-heading font-bold bg-purple-50 text-purple-600 px-3 py-1 rounded-full uppercase">🧬 Hỗn Hợp</span>
                         ) : (
                             <span className="text-[10px] font-heading font-bold bg-blue-50 text-ucmas-blue px-3 py-1 rounded-full uppercase">{ex.mode === 'nhin_tinh' ? '👁️ Nhìn' : ex.mode === 'nghe_tinh' ? '🎧 Nghe' : '⚡ Flash'}</span>
                         )}
                         <span className="text-[10px] font-heading font-bold bg-gray-100 text-gray-500 px-3 py-1 rounded-full uppercase">{ex.config?.digits || '?'}D{ex.config?.operands || '?'}R</span>
                      </div>
                      <button onClick={() => handleStartAssigned(ex)} className="w-full bg-gray-50 group-hover:bg-ucmas-blue group-hover:text-white text-gray-400 py-4 rounded-2xl font-heading font-black text-xs uppercase transition shadow-sm">Bắt đầu ôn luyện ➝</button>
                  </div>
              ))}
          </div>
      )}
