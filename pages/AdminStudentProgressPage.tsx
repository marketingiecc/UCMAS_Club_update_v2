...
const AdminStudentProgressPage: React.FC = () => {
  ...
  const activeFilters = useMemo(() => {
    ...
    if (!filterTeacherId && filterClassId) f.classId = filterClassId; // teacher filter takes precedence
      ...
      if (summaryRes.success) {
        ...
        const nextStudents: UserProfile[] = (summaryRes.data || []).map((r: any) => {
          ...
          return {
            ...
            // legacy fields left empty (will still render fine)
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-heading font-black text-gray-800 uppercase tracking-tight">
              Tra cứu tiến trình học sinh
            </h1>
            <p className="text-sm text-gray-500 mt-1">
              Lọc theo lớp / giáo viên / học sinh (mã, SĐT, tên). Bấm vào học sinh để xem chi tiết thống kê.
            </p>
          </div>
          <button
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-xs font-heading font-bold text-gray-600 mb-1">Theo giáo viên</label>
              <select
              ...
              <div className="text-[11px] text-gray-400 mt-1">
                Nếu chọn giáo viên, hệ thống sẽ hiển thị học sinh thuộc các lớp giáo viên đó đang quản lý.
              </div>
            </div>

            <div>
              <label className="block text-xs font-heading font-bold text-gray-600 mb-1">Theo lớp</label>
              <select
                value={filterClassId}
                onChange={(e) => setFilterClassId(e.target.value)}
                disabled={!!filterTeacherId}
                className="w-full px-4 py-3 rounded-2xl border border-gray-200 bg-white disabled:opacity-60"
              >
                <option value="">Tất cả</option>
                {classes.map((c) => (
              ...
              <div className="text-[11px] text-gray-400 mt-1">
                (Dùng bảng `classes` + `class_students`. Nếu chưa migrate từ `profiles.class_name` thì danh sách có thể trống.)
              </div>
            </div>
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-heading font-black text-gray-800">
              Danh sách học sinh ({loading ? '…' : students.length})
            </h2>
            <button
          ...
          {loading ? (
            ...
            <div className="text-gray-500 text-sm">Không có học sinh phù hợp bộ lọc.</div>
          ) : (
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <h3 className="text-sm font-heading font-black uppercase text-gray-700 mt-6 mb-2">Luyện theo lộ trình (learning paths)</h3>
