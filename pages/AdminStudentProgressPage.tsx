  ...
  const refresh = async () => {
    ...
    try {
      ...
      if (summaryRes.success) {
        ...
        const cupCounts = await backend.getCupsCountsByUserIds(s.map(x => x.id));
        const withCups = s.map((st) => ({
          ...st,
          cups_count: typeof cupCounts[st.id] === 'number' ? cupCounts[st.id] : (st.cups_count || 0),
        }));
        const attemptStats = await backend.getStudentsAttemptsSummaryByUserIds({
          userIds: withCups.map(x => x.id),
          from,
          to,
        });
        const map: Record<string, any> = {};
        withCups.forEach((st) => {
          map[st.id] = {
            ...(attemptStats[st.id] || {}),
            cups_count: st.cups_count || 0,
            study_level_id: st.study_level_id || getStudyLevelIdFromLegacySymbol(st.level_symbol),
            exam_level_id: st.exam_level_id || undefined,
            level_symbol: st.level_symbol,
          };
        });
        setStudents(withCups);
        setSummaryByStudentId(map);
      }
    } finally {
      setLoading(false);
            ...
            {loading ? (
              ...
              <div className="min-w-[1300px] divide-y divide-gray-50">
                {students.map((s) => {
                  ...
                  return (
                    <div key={s.id} className="grid grid-cols-[repeat(13,minmax(0,1fr))] gap-4 px-6 py-4 items-center hover:bg-blue-50/30 transition-colors group">
                      {/* Student Info */}
