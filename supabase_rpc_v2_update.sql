-- Migration: Update rpc_get_students_progress_summary to include cups_count
-- Run this in Supabase SQL Editor.

-- [IMPORTANT] Drop the existing function first because the return type (columns) has changed.
-- We must match the signature exactly to drop it, or use the specific argument types.
...
returns table (
  ...
  cups_count int -- [NEW] Added column
)
language plpgsql
stable
security definer
...
begin
  ...
  cups_agg as ( -- [NEW] Aggregation for cups
    select
      u.user_id as student_id,
      count(*)::int as cups_count
    from public.user_collected_cups u
    group by u.user_id
  )
  select
    p.id as student_id,
    ...
    coalesce(cu.cups_count, 0) as cups_count -- [NEW] Return cups_count
  ...
  left join cups_agg cu on cu.student_id = p.id -- [NEW] Join with cups_agg
