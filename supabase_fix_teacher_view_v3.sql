-- Fix: Allow Teachers to see students even if relational links (teacher_classes) are missing
-- Strategy:
-- 1. Auto-assign teachers to classes based on profile `class_name` if they have one (some teachers might be assigned to a "Class" in profile).
-- 2. Update RPC to be slightly more permissive or just rely on the sync script we already ran.
-- 3. Actually, the best fix is to ensure the Teacher is assigned to the Class.

-- [1] Auto-Assign Teachers to Classes based on text match (if they have a class_name in profile)
-- Only for roles 'teacher'
insert into public.teacher_classes (class_id, teacher_id, assigned_at)
select distinct
  c.id as class_id,
  p.id as teacher_id,
  now()
...
where p.role = 'teacher'
  ...
  and not exists (
    select 1 from public.teacher_classes tc where tc.teacher_id = p.id and tc.class_id = c.id
  )
on conflict (class_id, teacher_id) do nothing;

-- [2] Update RPC to select cups_count (it was missing in previous RPC definition!)
-- User said "reported incorrect counts".
-- Also, we need ensuring `cups_count` is returned.
...
begin
  ...
  return query
  with
  -- 1. Identify valid students (roster)
  roster as (
    select distinct p.id as student_id
    from public.profiles p
    where coalesce(p.role, 'student') <> 'admin'
  ),
  -- 2. Scoped students (Teacher -> Class -> Student)
  scoped as (
    select r.student_id
    from roster r
    where
      -- Filter by Class ID if provided (dropdown)
      (
        p_class_id is null
        or exists (
      ...
      and
      -- Filter by Teacher Scope (if teacher login)
      (
        ...
        -- FALLBACK: If teacher has NO assigned classes, maybe they are just viewing all students? 
        -- No, strict security says NO.
        -- But for debugging, if `teacher_classes` is empty, maybe we should relax?
        -- No, let's keep it strict but rely on the Fix Script to populate assignments.
      )
  ),
  ...
  left join cups_agg cu on cu.student_id = p.id  -- Join cups
