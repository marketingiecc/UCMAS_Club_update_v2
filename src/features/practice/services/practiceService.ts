...
export const practiceService = {
  ...
  async adminDeleteExercise(examId: string) {
    try {
      // Delete exam - mixed_questions should cascade or handle manually
      const { error } = await supabase.from('assigned_practice_exams' as any).delete().eq('id', examId);
